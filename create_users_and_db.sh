...
for i in $(seq 1 11); do
  ...
  mysql -e "CREATE USER 'u$i'@'localhost' IDENTIFIED BY '$password';"
  mysql -e "GRANT ALL PRIVILEGES ON DB$i.* TO 'u$i'@'localhost';"
  mysql -e "REVOKE CREATE, DROP ON DB$i.* FROM 'u$i'@'localhost';"
  ...
  mysql -e "INSERT INTO DB.Users (username, password) VALUES ('u$i', '$encrypted_password');"
done
